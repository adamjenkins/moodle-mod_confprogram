...
- **Review phase**: assign reviewers (individually or via reviewer groups) to submissions, review using a rubric (built on Moodle's core advanced grading API), optionally blind (hiding submitter/reviewer identities from each other), flag panels/keynotes as "unvetted" (excluded from review), and record a final Accept/Reject/Resubmit/Waitlist decision. "Resubmit" reopens the submission for editing with reviewer feedback visible, for a second review round. Decisions are recorded from the **Decision report** (`decisions.php`): a filterable table (by track, and by decision status -- no decision yet / accepted / rejected / resubmit / waitlisted) of every non-unvetted submission with its current round's completed reviews, supporting both a per-row single decision and a bulk decision (checked rows, or a header "select all") applied in one go behind a confirm dialog naming the chosen decision and how many submissions it affects. A single bulk **"Start a new round"** link, shown whenever at least one submission is resubmit-decided, replaces per-submission navigation and lands directly on `assign.php`'s existing bulk-assign checkboxes, pre-filtered to every resubmit-decided submission via `?resubmitted=1`.
