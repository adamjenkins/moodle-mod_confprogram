...
- Moodle のアドバンスト評定 API を用いた**ルーブリックで査読**します。任意で**ブラインド**（査読者と著者の氏名を相互に非表示。著者を特定しうる主催者定義の応募フィールドも同様に非表示）にできます。既定では **Manager** ロールのみがブラインド査読を回避できます。他のロールにも査読中に身元を見せたい場合は、そのロールに `mod/confprogram:viewidentity` を明示的に付与してください。
...
- 採択された応募を、レスポンシブで絞り込み可能なリストとして表示し、**お気に入り**の星を備えます。Conference Scheduler の時刻・会場と「マイタイムテーブル」の状態と同期します。複数日にまたがる場合は、列幅を揃えた単一のアクセシブルな表として表示され、日付の区切り行が挿入されます。狭い画面ではモバイル向けの2行レイアウトに折りたたまれます。`?trackid=X` で1つのトラックに絞り込めます（スケジューラーのトラックピルのリンク先です）。
- **表示設定**（どちらのフェーズでも編集モードから開けます）で、リストと詳細モーダルに表示する応募フィールドを選べます。
